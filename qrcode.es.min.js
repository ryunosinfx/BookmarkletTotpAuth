...
class QR8bitByte {
	constructor(data) {
		...
		if (this.parsedData.length !== data.length) {
			this.parsedData.unshift(191); // ADD BOM
...
class QRCodeModel {
	...
	makeImpl(test, maskPattern) {
		const moduleCount = this.typeNumber * 4 + 17,
			moduleCountOffsetMinus7 = moduleCount - 7,
		...
		for (let rowIndex = 0; rowIndex < moduleCount; rowIndex++) {
			...
			for (let colIndex = 0; colIndex < moduleCount; colIndex++) row[colIndex] = null;
		}
	...
	setupPositionProbePattern(row, col) {
		...
		for (let r = -1; r <= 7; r++) {
			const rowR = row + r;
			if (rowR <= -1 || moduleCount <= rowR) continue;
			for (let c = -1; c <= 7; c++) {
				const colC = col + c;
				if (colC <= -1 || moduleCount <= colC) continue;
				models[rowR][colC] =
					!!(0 <= r && r <= 6 && (c === 0 || c === 6)) ||
					(0 <= c && c <= 6 && (r === 0 || r === 6)) ||
					(2 <= r && r <= 4 && 2 <= c && c <= 4);
		...
		for (let i = 0; i < pl; i++)
			for (let j = 0; j < pl; j++) {
				const rowIndex = pos[i],
					colIndex = pos[j];
				if (modules[rowIndex][colIndex] != null) continue;
				for (let r = -2; r <= 2; r++) {
	...
	static createData(typeNumber, errorCorrectLevel, dataList) {
		...
		for (let rsBlock of rsBlocks) totalDataCount += rsBlock.dataCount;
		const totalDataBitCount = totalDataCount * 8,
			bitLength = buffer.getLengthInBits();
		if (bitLength > totalDataBitCount)
			throw new Error('code length overflow. (' + bitLength + '>' + totalDataBitCount + ')');
		if (bitLength + 4 <= totalDataBitCount) buffer.put(0, 4);
		while (buffer.getLengthInBits() % 8 !== 0) buffer.putBit(false);
		while (true) {
			if (buffer.getLengthInBits() >= totalDataBitCount) break;
			buffer.put(PAD0, 8);
			if (buffer.getLengthInBits() >= totalDataBitCount) break;
			buffer.put(PAD1, 8);
		}
	...
	static createBytes(buffer, rsBlocks) {
		...
		const bitBuffer = buffer.buffer,
			len = rsBlocks.length,
			dcdata = new Array(len),
			ecdata = new Array(len);
		let totalCodeCount = 0;
		for (let r = 0; r < len; r++) {
			const rsBlock = rsBlocks[r],
				dcCount = rsBlock.dataCount,
				ecCount = rsBlock.totalCount - dcCount,
				dcdataRow = new Array(dcCount);
			maxDcCount = Math.max(maxDcCount, dcCount);
			maxEcCount = Math.max(maxEcCount, ecCount);
			for (let i = 0; i < dcCount; i++) dcdataRow[i] = 0xff & bitBuffer[i + offset];
			dcdata[r] = dcdataRow;
			offset += dcCount;
			const rsPoly = QRUtil.getErrorCorrectPolynomial(ecCount),
				ecLen = rsPoly.getLength() - 1,
				rawPoly = new QRPolynomial(dcdataRow, ecLen),
				modPoly = rawPoly.mod(rsPoly),
				modLen = modPoly.getLength(),
				offsetLen = modLen - ecLen,
				ecdataRow = new Array(ecLen);
		...
		for (let i = 0; i < maxDcCount; i++)
			for (let r = 0; r < len; r++) {
				...
				if (i < dcdataRow.length) data[index++] = dcdataRow[i];
			}
		for (let i = 0; i < maxEcCount; i++)
			for (let r = 0; r < len; r++) {
				const ecdataRow = ecdata[r];
				if (i < ecdataRow.length) data[index++] = ecdataRow[i];
			}
		return data;
	}
}
	...
	getLostPoint(qrCode) {
		...
		for (let row = 0; row < moduleCount; row++)
			for (let col = 0; col < moduleCount; col++) {
				...
				for (let r = -1; r <= 1; r++) {
					const rowR = row + r;
					if (rowR < 0 || moduleCount <= rowR) continue;
					for (let c = -1; c <= 1; c++) {
						const colC = col + c;
						if (colC < 0 || moduleCount <= colC) continue;
						if (r === 0 && c === 0) continue;
						if (isDark === qrCode.isDark(rowR, colC)) sameCount++;
					}
				}
...
class QRMath {
	...
	static init() {
		...
		for (let i = 0; i < 8; i++) expTable[i] = 1 << i;
		for (let i = 8; i < 256; i++)
...
class QRPolynomial {
	constructor(num, shift) {
		...
		if (numLen === undefined) throw new Error(numLen + '/' + shift);
		let offset = 0;
		while (offset < numLen && num[offset] === 0) offset++;
		const numLenOffset = numLen - offset,
			nums = new Array(numLenOffset + shift);
		for (let i = 0; i < numLenOffset; i++) nums[i] = num[i + offset];
		this.num = nums;
	}
	get(index) {
	...
	multiply(e) {
		const num = this.num,
			...
			eNum = e.num,
			eLen = eNum.length,
			numResult = new Array(numLen + eLen - 1);
	...
	mod(e) {
		const num = this.num,
			...
			eNum = e.num,
			eLen = eNum.length;
		if (numLen - eLen < 0) return this;
		const ratio = QRMath.glog(num[0]) - QRMath.glog(eNum[0]),
			numResult = new Array(numLen);
		for (let i = 0; i < numLen; i++) numResult[i] = num[i];
...
class QRRSBlock {
	...
	static getRSBlocks(typeNumber, errorCorrectLevel) {
		...
		for (let i = 0; i < length; i++) {
			const index = i * 3,
				count = rsBlock[index + 0],
				totalCount = rsBlock[index + 1],
				dataCount = rsBlock[index + 2];
			for (let j = 0; j < count; j++) rsBlocks.push(new QRRSBlock(totalCount, dataCount));
		}
		return rsBlocks;
	}
...
class SvgDrawer {
	...
	draw(qrCodeData) {
		...
		const nWidth = Math.floor(htmlOption.width / nCount),
			nHeight = Math.floor(htmlOption.height / nCount);
		this.clear();
		const attrs = {
			viewBox: '0 0 ' + nCount + ' ' + nCount,
			width: '100%',
			height: '100%',
			fill: htmlOption.colorLight,
		};
	...
	makeSVG(tag, attrs) {
		...
		for (const [key, value] of Object.entries(attrs)) if (value) elm.setAttribute(key, value);
		return elm;
	}
	clear() {
...
class HtmlDrawer {
	...
	draw(qrCodeData) {
		const htmlOption = this.htmlOption,
			elm = this.elm,
			nCount = qrCodeData.getModuleCount(),
			nWidth = Math.floor(htmlOption.width / nCount),
			nHeight = Math.floor(htmlOption.height / nCount),
			tElm = document.createElement('table');
		...
		elm.append(tElm);
		const elTable = elm.childNodes[0], // Fix the margin values as real size.
			nLeftMarginTable = (htmlOption.width - elTable.offsetWidth) / 2,
			nTopMarginTable = (htmlOption.height - elTable.offsetHeight) / 2;
...
class CanvasDrawer {
	...
	draw(qrCodeData) {
		...
		const imgElm = this.imgElm,
			ctx = this.ctx,
			nCount = qrCodeData.getModuleCount(),
			nWidth = htmlOption.width / nCount,
			nHeight = htmlOption.height / nCount,
		...
		for (let rowIndex = 0; rowIndex < nCount; rowIndex++) {
			for (let colIndex = 0; colIndex < nCount; colIndex++) {
				const isDark = qrCodeData.isDark(rowIndex, colIndex),
					nLeft = colIndex * nWidth,
					nTop = rowIndex * nHeight;
...
export class QRCode {
	constructor(elm, vOption = {}) {
		...
		if (vOption) for (let i in vOption) this.htmlOption[i] = vOption[i]; // Overwrites options
		const elment = typeof elm === 'string' ? document.getElementById(elm) : elm,
			DrawingClass = this.htmlOption.useSVG ? SvgDrawer : this.htmlOption.useHtml ? HtmlDrawer : CanvasDrawer;
		this.elm = elment;
		this.drawer = new DrawingClass(elment, this.htmlOption);
		if (this.htmlOption.text) this.makeCode(this.htmlOption.text);
	}
	/**
	 * Make the QRCode
	 *
	...
	makeCode(sText) {
		const correctLevel = this.htmlOption.correctLevel,
			qrCodeData = new QRCodeModel(this._getTypeNumber(sText, correctLevel), correctLevel);
	...
	_getTypeNumber(sText, nCorrectLevel) {
		...
		if (nType > limit) throw new Error('Too long data');
		return nType;
	}
	...
	setCorrectLevel(correctLevel = QRErrorCorrectLevel.H) {
		if (typeof correctLevel === 'string') {
			if (correctLevel === 'H') this.htmlOption.correctLevel = QRErrorCorrectLevel.H;
			else if (correctLevel === 'Q') this.htmlOption.correctLevel = QRErrorCorrectLevel.Q;
			else if (correctLevel === 'M') this.htmlOption.correctLevel = QRErrorCorrectLevel.M;
			else if (correctLevel === 'L') this.htmlOption.correctLevel = QRErrorCorrectLevel.L;
		} else this.htmlOption.correctLevel = correctLevel;
	}
	/**
	 * Clear the QRCode
	 */
