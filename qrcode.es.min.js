...
const QRUtil = {
	...
	getMask(mP, i, j) {
		...
		switch (mP) {
			...
			default:
				throw new Error('bad maskPattern:' + mP);
		}
	},
...
class QRPolynomial {
	constructor(n, s) {
		const nL = n.length;
		if (nL === undefined) throw new Error(nL + '/' + s);
		let o = 0;
		while (o < nL && n[o] === 0) o++;
		const nLO = nL - o,
			ns = new Array(nLO + s);
...
class QRRSBlock {
	...
	static getRSBlocks(tN, eCL) {
		...
		if (rsB === undefined) throw new Error('bad rs block @ typeNumber:' + tN + '/errorCorrectLevel:' + eCL);
...
export class QRCode {
	...
	_getTypeNumber(s, nCL) {
		...
		if (nT > l) throw new Error('Too long data');
		return nT;
	}
	_getUTF8Length(s) {
